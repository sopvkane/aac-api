...
/**
 * Controller "contract" test using standalone MockMvc:
 * - no SpringBoot context
 * - no DB / JPA
 * - no Docker / Testcontainers
 * - verifies JSON + status codes + filtering params are passed to service
 */
    ...
    void create_then_list_filters_work() throws Exception {
        ...
        mvc.perform(post("/api/phrases")
                        ...
                        .content(objectMapper.writeValueAsString(
                                new CreatePhraseRequest("Hello world", "greeting")
                        )))
        ...
        mvc.perform(post("/api/phrases")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(
                                new CreatePhraseRequest("I want tea", "needs")
                        )))
                .andExpect(status().isCreated())
        ...
        // bonus: ensure controller passed the right optionals to service
